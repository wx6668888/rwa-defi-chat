    ...
    <div className="min-h-screen bg-void-black pb-20">
        ...
        <div className="flex items-center gap-4 px-6 py-4">
          ...
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search..." autoFocus
              className="w-full bg-surface-1 border border-border-subtle rounded-xl pl-11 pr-4 py-3 text-sm text-text-primary" />
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-6">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {recent.map((term) => (
            <button key={term} className="px-3 py-1.5 bg-surface-1 border border-border-subtle rounded-full text-xs text-text-secondary">
              {term}
            </button>
          ))}
        </div>
